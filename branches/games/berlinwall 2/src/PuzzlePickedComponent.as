package  
{
	...
	public class PuzzlePickedComponent extends TickedComponent 
	{
		...
		private var in_hint:Boolean = false;
		...
		override protected function onAdd():void 
		{
			...
			status.setProperty(new PropertyReference("@puzzle_status.steps"), 0);
			...
			PBE.mainStage.addEventListener(MouseEvent.CLICK, onClick, false, 0, true);
			PBE.mainStage.addEventListener("HINT", onHint, false, 0, true);
			PBE.mainStage.addEventListener("HINT_FINISHED", onHintFinished, false, 0, true);
		}
		
		private function onHintFinished(e:Event):void 
		{
			in_hint = false;
		}
		...
		private function onHint(e:Event):void 
		{
			in_hint = true;
			var status:IEntity = PBE.lookupEntity('status');
			status.setProperty(new PropertyReference("@puzzle_status.switch_stack"),[]);
		}
		...
		private function onClick(e:MouseEvent):void 
		{
			try{
				var status:IEntity = PBE.lookupEntity('status');
				var isPaused:Boolean = status.getProperty(new PropertyReference("@puzzle_status.isPaused")) as Boolean;
				...
				var switch_stack:Array = status.getProperty(new PropertyReference("@puzzle_status.switch_stack"));
				...
				if(!isPaused&&!in_hint){
					...
					for each (var o:* in results) {
						if (o.owner.name != 'background'){
							if (owner.name == o.owner.name) {
								
								modifiers = [new ColorizeModifier([0.5, 0.5, 0.5, 0, 0], [0.5, 0, 0, 0, 0], [0.5, 0, 0, 0, 0], [0, 0, 0, 1, 0])];
								o.owner.setProperty(new PropertyReference("@render.modifiers"), modifiers);
								if (!is_in_stack(o.owner.name)) {
									PBE.log(this, "clicked");
									trace("Stack -->" + switch_stack);
									if (switch_stack.length < 2) {
										switch_stack.push(owner.name);
										status.setProperty(new PropertyReference("@puzzle_status.switch_stack"), switch_stack);
										if (switch_stack.length == 2) {
											PBE.log(this, "need switch --> " + switch_stack.toString());
											try {
												do_switch();
											}catch (e:Error) {
												PBE.log(this, e.message);
											}
										}
									}else {
		...
		private function is_in_stack(name:String):Boolean {
			var status:IEntity = PBE.lookupEntity('status');
			var switch_stack:Array = status.getProperty(new PropertyReference("@puzzle_status.switch_stack"));
			for each(var n:String in switch_stack) {
				if (n == name) {
					return true;
				}
			}
		...
		private function do_switch():void {
			...
			//trace(switch_stack.length);
			
			switch_queue = [switch_stack[0],switch_stack[1]];
			switch_queue_index = [];
			for each(var name:String in switch_stack){
			...
			if (switch_queue.length == 2) {
				...
				status.setProperty(new PropertyReference("@puzzle_status.steps"), status.getProperty(new PropertyReference("@puzzle_status.steps"))+1);
			}
			//trace(stack);
			try {
			...
			for each(var nn:String in switch_stack) {
				var obj1:IEntity = PBE.lookupEntity(nn);
				obj1.setProperty(new PropertyReference("@render.modifiers"), []);
			}
			switch_stack = [];
			status.setProperty(new PropertyReference("@puzzle_status.switch_stack"), []);
			trace(switch_stack.length);
		...
		private function onRelease(e:MouseEvent):void 
		{
			try{
				...
				if(!isFinished){
					if (is_busy) {
						PBE.mainStage.dispatchEvent(new Event("PIECE_RELEASED"));
						results = [];
						switch_queue = [];
						switch_queue_index = [];
						pos = new Point(PBE.mainStage.mouseX, PBE.mainStage.mouseY);
		...
		protected override function onRemove():void {
			...
			PBE.mainStage.removeEventListener("HINT_FINISHED", onHintFinished);
			PBE.mainStage.removeEventListener("HINT", onHint);
			...
			if(!isPaused){
				...
				for each (var o:* in results) {
					if (o.owner.name != 'background'){
						...
						var modifiers:Array = [];
						if (owner.name == o.owner.name&&!is_in_stack(o.owner.name)) {
							modifiers = [new ColorizeModifier([0.5, 0, 0, 0, 0], [0.5, 0, 0, 0, 0], [0.5, 0, 0, 0, 0], [0, 0, 0, 1, 0])];
							o.owner.setProperty(new PropertyReference("@render.modifiers"), modifiers);
							for each(var puzzle:String in stack) {
								
								if (o.owner.name != puzzle) {
									var obj:IEntity = PBE.lookupEntity(puzzle);
									if (!is_in_stack(o.owner.name)) {
										//trace("reset hover " + o.owner.name);
										obj.setProperty(new PropertyReference("@render.modifiers"), []);
									}
								}
							}
						}else {
							var switch_stack:Array = status.getProperty(new PropertyReference("@puzzle_status.switch_stack"));
							for each(var name:String in switch_stack) {
								var obj1:IEntity = PBE.lookupEntity(name);
								var mods:Array = [new ColorizeModifier([0.5, 0.5, 0.5, 0, 0], [0.5, 0, 0, 0, 0], [0.5, 0, 0, 0, 0], [0, 0, 0, 1, 0])];
								obj1.setProperty(new PropertyReference("@render.modifiers"), mods);
							}
							//modifiers = [];
						}
						//trace(modifiers);
