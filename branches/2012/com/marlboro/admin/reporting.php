...
class reporting extends SQLData{
	...
	function admin(){
		...
		}elseif( $act == 'ba-entourage' ){
			return $this->ba_entourage();
		}elseif( $act == 'social_redeem' ){
			return $this->social_redeem();
	...
	function ba_entourage(){
		$save = $this->Request->getPost('save');
		
		if($save==1){		
			if ($_FILES['xls_a']['name']!='') {
				$type = $_FILES['xls_a']['name'];
			} elseif($_FILES['xls_b']['name']!='') {
				$type = $_FILES['xls_b']['name'];
			} else {
				$type = $_FILES['xls_c']['name'];
			}
			
			list($filename,$ext) = explode('.',$type);
			if ($ext!='xls') {
				$err = 'Please upload XLS file!';
			} else {				
				if ($_FILES['xls_a']['name']!='') {				
					$data = new Spreadsheet_Excel_Reader($_FILES['xls_a']['tmp_name']);
					$badge_type = '9';
					$text = 'Type A - 9 badges';				
				} elseif($_FILES['xls_b']['name']!='') {
					$data = new Spreadsheet_Excel_Reader($_FILES['xls_b']['tmp_name']);
					$badge_type = '18';
					$text = 'Type A - 18 badges';
				
				} else {
					$data = new Spreadsheet_Excel_Reader($_FILES['xls_c']['tmp_name']);
					$badge_type = '32';
					$text = 'Type A - 32 badges';
				}
				...
				// import data excel mulai baris ke-1
				for ($i=1; $i<=$baris; $i++) {
					$now = date("Y-m-d H:i:s");
					// data email (kolom ke-1)
					$email = $data->val($i,1);
					$query = "INSERT IGNORE INTO marlboro_connection_report_2012.tbl_ba_entourage (email,date_time,badge_type) VALUES ('$email','$now','$badge_type')";

					// jika proses insert data sukses, maka counter $sukses bertambah
					// jika gagal, maka counter $gagal yang bertambah
					if ($this->query($query)) {
						$sukses++;
						$err = "<h3>Proses import data ".$text." selesai.</h3>";
						$err .= "<p>Jumlah data yang sukses diimport : ".$sukses."<br>";
					} else {
						$gagal++;
						$err = "Jumlah data yang gagal diimport : ".$gagal."</p>";
					}
				}
			}
		}
		...
		return $this->View->toString("marlboro/admin/ba-entourage.html");
	}
...
GROUP BY prize;";
	...
	function social_redeem(){
		...
		if ($status==0){ //default
			...
			$filter = "AND a.n_status = '0'";
		} else { //Redeem
			$filter = "AND a.n_status = '1'";
		}
		
		if ($_POST['status']==$status) {
			$qry = "SELECT count(*) total FROM social_redeem WHERE 1 ORDER BY submit_date;";
			$list = $this->fetch($qry);
			$total = $list['total'];
			$total_per_page = 30;
			
			$qry = "SELECT a.id,b.name,b.email,a.street,a.complex,a.province,a.city,a.phone,a.mobile,a.prize,a.submit_date,a.n_status
					FROM social_redeem a
					LEFT JOIN social_member b ON a.register_id = b.register_id
					WHERE 1 AND a.n_status <> '2'
					$filter
					ORDER BY submit_date DESC LIMIT $start,$total_per_page;
			";
		}
		...
		$this->View->assign('_status',$status);
		$this->Paging = new Paginate();
		$this->View->assign("paging",$this->Paging->getAdminPaging($start, $total_per_page, $total, "?s=reporting&act=social_redeem&status=".$this->Request->getPost("status")));
		return $this->View->toString("marlboro/admin/reporting_redeem.html");
	}
	...
	function getCSV(){
		...
		GROUP BY prize;";
			...
			}elseif($data == 'social_redeem'){
				$status = $_GET['status'];
				if ($status==0){ //default
					$filter = "";
				} elseif($status==1) { //News
					$filter = "AND a.n_status = '0'";
				} else { //Hunting
					$filter = "AND a.n_status = '1'";
				}
				...
				$sql = "
					SELECT a.id,b.name,b.email,a.street,a.complex,a.province,a.city,a.phone,a.mobile,a.prize,a.submit_date,a.n_status
					FROM social_redeem a
					LEFT JOIN social_member b ON a.register_id = b.register_id
					WHERE 1 AND a.n_status <> '2'
					$filter
				...
				";
				$rs = $this->fetch($sql,1);
				$str = "\"ID\";\"Name\";\"Email\";\"Street\";\"Zip Code\";\"Province\";\"City\";\"Phone\";\"Mobile\";\"Prize\";\"Submit Date\";\"Status\"\r\n";
				$num = count($rs);
				for($i=0;$i<$num;$i++){
					$str .= "\"".$rs[$i]['id']."\";\"".$rs[$i]['name']."\";\"".$rs[$i]['email']."\";\"".$rs[$i]['street']."\";\"".$rs[$i]['complex']."\";\"".$rs[$i]['province']."\";\"".$rs[$i]['city']."\";\"".$rs[$i]['phone']."\";\"".$rs[$i]['mobile']."\";\"".$rs[$i]['prize']."\";\"".$rs[$i]['submit_date']."\";\"".$rs[$i]['n_status']."\"\r\n";
				}				
